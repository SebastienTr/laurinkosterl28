...
![Deck fittings and winch positions](images/deck_rigging.png)
...
The **mainsheet traveller** fits across the aft timber cap, below the tiller. Its two 0.9 mm locating pins fit the 1.15 mm timber sockets. Glue it after fitting the joined timber assembly. The centre car has an eyelet pilot hole. It is fixed for this display model.
...
There are two separate dark winches on the forward cockpit coamings. Each uses a 1.2 mm locating pin and a 1.5 mm socket in its timber backing pad. Lead each genoa sheet from the clew, through the eye in its side track car, then around the winch. The timber backing pads are small model assembly allowances, not surveyed fittings on the boat. The winch's top socket is a detail; wrap the sheet around the drum. Track cars are fixed and have 0.8 mm eyelet pilots. The tracks have locating pins and matching deck sockets.
