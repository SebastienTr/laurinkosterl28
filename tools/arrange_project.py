...
with zipfile.ZipFile(R/'bambu/painted.3mf') as zi:
 ...
 for n,plate in enumerate(cfg.findall('plate')):
  ...
  for inst in plate.findall('model_instance'):
   oid=inst.find("metadata[@key='object_id']").get('value');item=build[oid];tr=np.array(list(map(float,item.get('transform').split())));tr[9]+=20;item.set('transform',' '.join(f'{v:.10g}' for v in tr))
